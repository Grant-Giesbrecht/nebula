...
`model` has no GUI dependency and is always importable. The PySide6 view
lives in `app`; importing it (or calling launch()) requires PySide6, so it's
deferred rather than imported at package load.
"""
...
def launch(archive, archive_label=None):
    ...
    Imports PySide6 lazily so `import nebula.navigator` stays lightweight."""
